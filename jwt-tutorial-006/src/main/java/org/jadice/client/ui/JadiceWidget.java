...
import com.levigo.jadice.web.client.tools.AnnotationTool;
...
import com.levigo.jadice.web.client.ui.AnnotationProfileAwareToolbar;
import com.levigo.jadice.web.client.ui.AnnotationProfileAwareToolbar.Orientation;
...
import com.levigo.jadice.web.client.util.action.KeyStroke.Modifier;
...
public class JadiceWidget implements IsWidget, RequiresResize {
	...
	private void init() {
		...
		AnnotationProfileAwareToolbar annoBar = createAnnoToolbar();
		panel.addWest(annoBar, annoBar.getSize());
		panel.add(viewer);
	...
	private AbstractBar createToolbar(final Context context) {
		...
		toolbar.add(new JadiceDefaultButton(
				CustomActions.openDocumentAction(new KeyStroke(Keys.O, Modifier.CTRL), context)));
	...
	private AnnotationProfileAwareToolbar createAnnoToolbar() {
		return new AnnotationProfileAwareToolbar("Tutorial-Annotations", Orientation.VERTICAL);
	}
